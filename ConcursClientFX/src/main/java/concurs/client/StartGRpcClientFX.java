...
public class StartGRpcClientFX extends Application {
  private Stage primaryStage;

  private static int defaultChatPort = 55556;
  private static String defaultServer = "127.0.0.1";

  private static Logger logger = LogManager.getLogger(StartGRpcClientFX.class);

  public void start(Stage primaryStage) throws Exception {
    logger.debug("In start");
    Properties clientProps = new Properties();
    try {
      clientProps.load(StartGRpcClientFX.class.getResourceAsStream("/concursclient.properties"));
    ...
    /// TODO
    // atentie trebuie sa modific si portul
//    serverPort = 7060;
//    ConcursServicesGrpcProxy server = new ConcursServicesGrpcProxy(serverIP, serverPort);
//
    ConcursServicesRpcProxy server = new ConcursServicesRpcProxy(serverIP, serverPort);
